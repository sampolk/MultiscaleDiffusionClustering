function [acc,labels,t] = nfindri_lund(X, k, h, w, G, T, Y, flag)
...
if flag == 0
    ...
    for i = 1:length(T)
        [NFINDR,~] = EIA_NFINDR(X,k+2,200);
        [X_nfindr] = hyperNnls(X, NFINDR);
        
        X_nfindr = reshape(X_nfindr', h, w, k+2);
        X_nfindr = X_nfindr(:,:,3:k+2);
...
elseif flag == 1
    ...
    for i = 1:length(T)
        [NFINDR,~] = EIA_NFINDR1(X(:,Y~=1),k+2,200);
        [X_nfindr] = hyperNnls(X, NFINDR);
        X_nfindr = reshape(X_nfindr', h, w, k+2);
        X_nfindr = X_nfindr(:,:,3:k+2);
